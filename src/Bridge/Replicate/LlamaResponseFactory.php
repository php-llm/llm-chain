...
final readonly class LlamaResponseFactory implements ResponseFactory
{
    ...
    public function create(Model $model, object|array|string $input, array $options = []): ResponseInterface
    {
        ...
        return $this->client->request(sprintf('meta/meta-%s', $model->getVersion()), 'predictions', [
            'system' => self::convertMessage($input->getSystemMessage() ?? new SystemMessage('')),
            'prompt' => self::convertToPrompt($input->withoutSystemMessage()),
        ]);
    }
